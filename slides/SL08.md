...
<p class="text-center" style="font-size: 1.2rem;">値の組み<MathInline expr="(\mathbf{a}, b)"/>が<MathInline expr="b=\Sigma_{i=0}^{k-1} a_is_i+e \text{ mod } q"/>を満たすか、<br>ただのランダムな値の組み合わせなのか判定する問題</p>
</div>


ここで、整数$k\ge 1$, $q \ge 2$, $\mathbf{s}\in\mathbb{Z}^k_q$, および$\mathbb{Z}$上の確率分布$\chi$に対して，$\mathbf{a}\in \mathbb{F}_q^k$を一様ランダムに取り，$e\in \mathbb{Z}_q$を$\chi$に従ってとっている。
