...
<span class="text-amber-600 font-black mr-1">&#9654;</span>要点：World ID は単一プリミティブではなく <strong>MPC（秘匿照合）∘ co-SNARK（計算の検証可能性）∘ ZK（匿名性）</strong> の合成。<br />土台の一意性照合は MPC が担い、その正しさを co-SNARK が公開検証可能にし、発行後の利用は ZK で匿名化する。
</div>
