...
【パイプライン】① Source: 普通のコード(Rust/C)を書く（回路の手書き不要）→ ② Compile: VM の ISA(多くは RISC-V; StarkNet は Cairo) にコンパイル → ③ Execute: エミュレータが実行トレースを 1 命令ずつ生成 → ④ Prove: トレースを算術化して STARK/SNARK で証明 → ⑤ Verify: 小さな proof を verifier が再実行せず安価に検証(on-chain も可)。
