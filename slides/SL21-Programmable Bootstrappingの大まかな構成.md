...
<p class="text-center">ノイズの溜まった暗号文<MathInline expr="(\mathbf{a}, b)"/>と暗号化された<MathInline expr="v(x)"/>を使って<MathInline expr="x^{-(b-\langle\mathbf a,\mathbf s\rangle)}v(x)"/>を計算する</p>
</div>
<br>
...
<p class="text-center">
回転した多項式<MathInline expr="x^{-(b-\langle\mathbf a,\mathbf s\rangle)}v(x)"/>の暗号文から定数項を暗号文の状態で抜き出す
</p>
</div>
<br>
<div class="text-center">↓</div>
