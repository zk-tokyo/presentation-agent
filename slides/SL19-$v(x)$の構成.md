...
ただし実用的なパラメータでは$q$が大きすぎるため、多項式の次数は$q$より小さい$n$にし、$\mathbf{a}$と$b$も$n$に合わせて変換する（リスケーリング）
...
テスト多項式を回転させる場合、
$\hat{b}=\lfloor b\frac{2n}{q}\rceil, \hat{\mathbf{a}}=\lfloor \mathbf{a}\frac{2n}{q}\rceil$と変換して、$x^{-(\hat{b}-\hat{\mathbf{a}})}v(x) \text{ mod }x^n+1$を計算する
