...
<div class="text-sm text-gray-500 mt-1 mb-3">実行を複数のテーブルに分け、<strong>lookup</strong> で Memory に束ね、<strong>GKR</strong> で結合して証明する。</div>
...
<div class="text-center text-sm text-gray-600 mt-3 max-w-6xl mx-auto">4 テーブルを lookup で Memory に接続。<br /><strong>Lookup</strong>で 1 サイクル<strong>わずか 5 要素</strong>のみコミット、多項式コミットは <strong>WHIR</strong></div>
