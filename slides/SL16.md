...
【証明】あとは「割り切れる（= 商多項式が存在し、しかも低次数）」ことを証明すればよい。STARK は多項式コミット + FRI による low-degree test でハッシュベース・透明（trusted setup 不要）・耐量子に実現。verifier は多項式を数点だけ評価して確認するので、元の実行を再現せず安価に検証でき、proof も小さい（on-chain 可）。RISC0 は最後に Groth16 で wrap して on-chain verify をさらに安くする。
