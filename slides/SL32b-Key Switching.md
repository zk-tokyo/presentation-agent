...
さらに、$ksk[i,j]=\text{LWE}_\mathbf{s}(s''_i q /B^{j+1})(0\le i \le n-1, 0 \le j \le l-1)$ (これをKey switching keyと呼ぶ。$\Delta$を使わないことに注意)

を考えると、目的の$\mathbf{s}$による$m$の暗号文$\text{LWE}_\mathbf{s}(\Delta m)$は次のように計算できる。
