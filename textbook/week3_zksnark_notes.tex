...
\begin{description}
  ...
  \item[Soundness / Knowledge soundness] 嘘の主張を通しにくく、有効なproofからwitnessの存在または知識を保証できること。
...
\]
となる関数を意味する。
対話型プロトコル \(\Pi=(P,V)\) が関係 \(R\) に対するzero-knowledge proofまたはargumentであるとは、典型的には次を満たすことをいう。
...
\begin{description}
  ...
  \item[Soundness]
  \(x\notin L_R\) のとき、効率的な不正証明者 \(P^\ast\) が検証者を受理させる確率は高々 \(\negl(\lambda)\) である。
  \[
    \Pr[\langle P^\ast,V\rangle(x)=1]\le \negl(\lambda)
  \]
  argumentでは、この条件を計算量的な攻撃者に対して要求する。

  \item[Zero-knowledge]
  任意の効率的な不正検証者 \(V^\ast\) に対して、効率的なシミュレータ \(S\) が存在し、
  \[
    \View_{V^\ast}^{P(w)}(x) \stackrel{c}{\approx} S(x)
  \]
  が成り立つ。
  左辺は本物の証明で検証者が見る会話全体、右辺はwitnessを使わずに生成した擬似的な会話である。
  二つが計算量的に区別できないなら、検証者はwitness由来の追加情報を得ていないと考える。
\end{description}

SNARKでいうknowledge soundnessは、soundnessをさらに強めた性質である。
有効なproofを作れる証明者から、対応するwitnessを取り出す抽出器が存在する、という形で定式化する。
非対話型のZKPでは、検証者のviewはproof \(\pi\) そのものに近い。
その場合のzero-knowledge性は、実 proof の分布
\[
  \{\pi \leftarrow P(x,w)\}
\]
...
健全性の直感は、「証明者が \(b=0\) と \(b=1\) の両方に答えられるなら \(x\) を取り出せる」ことである。
同じ \(a\) に対して
...
\begin{description}
  ...
  \item[proof / argument] witnessを直接明かさず、statementが正しいことを示す証拠。SNARKでは計算量的健全性に基づくargumentと呼ぶのが厳密である。
